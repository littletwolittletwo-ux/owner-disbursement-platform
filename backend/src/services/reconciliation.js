...
export async function reconciliationSummary(month) {
  ...
  const [reservationResult, owners, unmatched] = await Promise.all([
    // Intersection rule: include bookings where BOTH checkout AND payout fall in this month
    query(`SELECT r.*, l.name listing_name, o.name owner_name, m.trust_transaction_id, t.amount actual_payout
           ...
           ORDER BY r.check_out`, [start, end, month, month]),
  ...
  // Build trust summary by channel — full amounts (intersection rule, no pro-rating)
  const channelTotals = {};
